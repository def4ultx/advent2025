...
let union_find size xs = 
  ...
  let rec dsu_union i j =
    ...
    if size - !counter = 1 && !first_pair = (0, 0) then
      begin
      first_pair := (i, j);
...
let size = read_file "day08_input.txt" |> List.length
...
let () = input
  ...
  |> fun (i, j) -> 
    ...
    let (ii, _, _) = List.nth input i in
    let (jj, _, _) = List.nth input j in

    let v = ii*jj in
    printf "First connected pair: %d,%d with value %d %d %d \n" i j ii jj v;
  (* |> printf "Total sum: %d\n" *)